...
st.write(f"""
Dataset ini terdiri dari {df.shape[0]} baris (objek) dan {df.shape[1]} kolom (atribut). Berikut adalah deskripsi dari beberapa kolom dalam dataset ini:
...
- Interpretasi: Grafik menunjukkan satu puncak besar, artinya sebagian besar peserta bekerja dalam kisaran jam tersebut.
""")
...
- Grafik batang di bawah ini menunjukkan rata-rata jam kerja per minggu di setiap divisi magang.
...
- Puncak (peak) dalam density plot menunjukkan rentang jam kerja yang paling sering dirasakan oleh peserta magang.
